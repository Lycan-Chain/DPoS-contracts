...
contract ValidatorSetAuRa is UpgradeabilityAdmin, IValidatorSetAuRa {
    ...
    mapping(uint256 => mapping(uint256 => mapping(uint256 => bool)))
        internal _maliceReportedForBlockMapped;
    ...
    function clearUnremovableValidator() external onlyInitialized {
        ...
        address unremovableStakingAddress = stakingAddressById[
            unremovableValidator
        ];
        require(
            msg.sender == unremovableStakingAddress || msg.sender == _admin()
        );
    ...
    function changeMiningAddress(address _newMiningAddress)
        external
        onlyInitialized
    {
        ...
        require(
            stakingContract.getDelegatorPoolsLength(_newMiningAddress) == 0
        );
        ...
        if (isValidatorById[poolId]) {
            ...
            require(
                _getCurrentBlockNumber() <
                    stakingContract.stakingEpochEndBlock().sub(
    ...
    /// delegator or a pool (staking address or mining address).
    function changeStakingAddress(address _newStakingAddress)
        external
        onlyInitialized
    {
        ...
        require(
            stakingContract.getDelegatorPoolsLength(_newStakingAddress) == 0
        );
    ...
    function newValidatorSet() external onlyBlockRewardContract {
        uint256[] memory poolsToBeElected = stakingContract
            .getPoolsToBeElected();
    ...
    function reportMalicious(
        ...
        uint256[] storage reportedValidators = _maliceReportedForBlock[
            maliciousId
        ][_blockNumber];

        reportedValidators.push(reportingId);
        _maliceReportedForBlockMapped[maliciousId][_blockNumber][
            reportingId
        ] = true;
    ...
    /// and should never be used as a pool or delegator before.
    function addPool(address _miningAddress, address _stakingAddress)
        external
        onlyStakingContract
        returns (uint256)
    {
    ...
    {
        return
            _getCurrentBlockNumber() <= bannedDelegatorsUntil(_miningAddress);
    }
    ...
    /// @param _miningAddress The mining address of the pool.
    function bannedDelegatorsUntil(address _miningAddress)
        public
        view
        returns (uint256)
    {
    ...
    function getPendingValidators() public view returns (address[] memory) {
        address[] memory miningAddresses = new address[](
    ...
    function isReportValidatorValid(
        address _miningAddress,
        bool _reportingValidator
        ...
        if (
            stakingContract.stakingEpoch() == 0 || validatorSetApplyBlock == 0
        ) {
            return isValid;
        }
        if (
            _getCurrentBlockNumber() - validatorSetApplyBlock <= MAX_VALIDATORS
        ) {
            ...
            bool previousValidator = _isValidatorPrevious[poolId] &&
                !isValidatorIdBanned(poolId);
    ...
    function reportMaliciousCallable(
        ...
        if (!isReportValidatorValid(_reportingMiningAddress, true))
            return (false, false);
        if (!isReportValidatorValid(_maliciousMiningAddress, false))
            return (false, false);
        ...
        if (validatorsNumber > 1) {
            ...
            uint256 reportsNumber = _reportingCounter[reportingId][
                currentStakingEpoch
            ];
            uint256 reportsTotalNumber = _reportingCounterTotal[
                currentStakingEpoch
            ];
    ...
    function validatorCounter(address _miningAddress)
        public
        view
        returns (uint256)
    {
    ...
    function validatorsToBeFinalized()
        public
        view
        returns (address[] memory miningAddresses, bool forNewEpoch)
    {
    ...
    function _applyMiningAddressChangeRequest() internal {
        ...
        if (poolId != 0) {
            ...
            address newMiningAddress = miningAddressChangeRequest
                .newMiningAddress;
    ...
    function _setPendingValidators(uint256[] memory _poolIds) internal {
        if (_poolIds.length == 0) {
            ...
            for (uint256 i = 0; i < _pendingValidators.length; i++) {
                ...
                _pendingValidators[i] = _pendingValidators[
    ...
    /// @param _stakingAddress The staking address of the newly created pool. Cannot be equal to the `_miningAddress`
    /// and should never be used as a pool or delegator before.
    function _addPool(address _miningAddress, address _stakingAddress)
        internal
        returns (uint256)
    {
        require(miningAddressChangeRequest.poolId == 0);
        ...
        uint256 poolId = uint256(address(_stakingAddress));
    ...
    function _banUntil() internal view returns (uint256) {
        uint256 blocksUntilEnd = stakingContract.stakingEpochEndBlock() -
            _getCurrentBlockNumber();
        // ~90 days, at least 12 full staking epochs (for 5 seconds block)
        return
            _getCurrentBlockNumber() +
            12 *
            stakingContract.stakingEpochDuration() +
            blocksUntilEnd;
    }
