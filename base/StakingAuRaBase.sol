...
contract StakingAuRaBase is UpgradeableOwned, IStakingAuRa {
    ...
    mapping(address => mapping(uint256 => uint256))
        internal _delegatorPoolsIndexes;
    ...
    event AddedPool(
        ...
        address indexed poolMiningAddress,
        uint256 poolId
    );
    ...
    function addPool(uint256 _amount, address _miningAddress)
        external
        payable
        returns (uint256)
    {
    ...
    function clearUnremovableValidator(uint256 _unremovablePoolId)
        external
    ...
    function initialValidatorStake(uint256 _totalAmount) external onlyOwner {
        ...
        for (uint256 i = 0; i < _pools.length; i++) {
            ...
            address stakingAddress = validatorSetContract.stakingAddressById(
                poolId
            );
    ...
    function removeMyPool() external onlyInitialized {
        ...
        require(
            stakingEpoch > 0 || !validatorSetContract.isValidatorById(poolId)
        );
    ...
    function setStakingEpochStartBlock(uint256 _blockNumber)
        external
        onlyValidatorSetContract
    {
    ...
    function moveStake(
        ...
        uint256 fromPoolId = validatorSetContract.idByStakingAddress(
            _fromPoolStakingAddress
        );
        uint256 toPoolId = validatorSetContract.idByStakingAddress(
    ...
    function stake(address _toPoolStakingAddress, uint256 _amount)
        external
        payable
    {
    ...
    function orderWithdraw(address _poolStakingAddress, int256 _amount)
        ...
        onlyInitialized
    {
        uint256 poolId = validatorSetContract.idByStakingAddress(
            _poolStakingAddress
        );
        ...
        address staker = msg.sender;
        address delegatorOrZero = (staker != _poolStakingAddress)
            ? staker
            : address(0);

        require(_isWithdrawAllowed(poolId, delegatorOrZero != address(0)));

        uint256 newOrderedAmount = orderedWithdrawAmount[poolId][
            delegatorOrZero
        ];
        ...
        if (staker == _poolStakingAddress) {
            ...
            uint256 unremovablePoolId = validatorSetContract
    ...
    function claimOrderedWithdraw(address _poolStakingAddress) external {
        ...
        orderedWithdrawAmountTotal[poolId] = orderedWithdrawAmountTotal[poolId]
            .sub(claimAmount);
    ...
    /// @param _delegator The delegator address.
    function getDelegatorPoolsLength(address _delegator)
        external
        view
        returns (uint256)
    {
    ...
    function areStakeAndWithdrawAllowed() public view returns (bool) {
        ...
        uint256 allowedDuration = stakingEpochDuration -
        ...
        returns (uint256)
    {
        ...
        address delegatorOrZero = (_staker != _poolStakingAddress)
            ? _staker
            : address(0);
    ...
    function poolDelegators(uint256 _poolId)
        public
        view
    ...
    /// but not yet claimed.
    /// @param _poolId The pool id.
    function poolDelegatorsInactive(uint256 _poolId)
        public
        view
        returns (address[] memory)
    {
    ...
    function _deletePoolToBeElected(uint256 _poolId) internal {
        ...
        if (
            ...
            _poolsToBeElected[indexToDelete] == _poolId
        ) {
    ...
    function _deletePoolToBeRemoved(uint256 _poolId) internal {
        ...
        if (
            ...
            _poolsToBeRemoved[indexToDelete] == _poolId
        ) {
    ...
    function _removePoolInactive(uint256 _poolId) internal {
        ...
        if (
            ...
            _poolsInactive[indexToRemove] == _poolId
        ) {
    ...
    function _addPool(
        ...
        uint256 poolId = validatorSetContract.addPool(
            _miningAddress,
            _stakingAddress
        );
    ...
    /// @param _poolId The pool id.
    /// @param _delegator The delegator's address.
    function _addPoolDelegatorInactive(uint256 _poolId, address _delegator)
        internal
    {
    ...
    /// @param _poolId The pool id.
    /// @param _delegator The delegator's address.
    function _removePoolDelegator(uint256 _poolId, address _delegator)
        internal
    {
        address[] storage delegators = _poolDelegators[_poolId];
    ...
    /// @param _poolId The pool id.
    /// @param _delegator The delegator's address.
    function _removePoolDelegatorInactive(uint256 _poolId, address _delegator)
        internal
    {
        address[] storage delegators = _poolDelegatorsInactive[_poolId];
    ...
    function _snapshotDelegatorStake(uint256 _poolId, address _delegator)
        internal
    {
        uint256 nextStakingEpoch = stakingEpoch + 1;
        uint256 newAmount = stakeAmount[_poolId][_delegator];

        delegatorStakeSnapshot[_poolId][_delegator][
            nextStakingEpoch
        ...
        stakeLastEpoch[_poolId][_delegator] = (newAmount == 0)
            ? nextStakingEpoch
            : 0;
    }
    ...
    function _stake(
        ...
        address delegatorOrZero = (_staker != _poolStakingAddress)
            ? _staker
            : address(0);
        uint256 newStakeAmount = stakeAmount[poolId][delegatorOrZero].add(
            _amount
        );
        ...
        stakeAmount[poolId][delegatorOrZero] = newStakeAmount;
        _stakeAmountByEpoch[poolId][delegatorOrZero][
            stakingEpoch
        ...
        if (_staker == _poolStakingAddress) {
            ...
            _addPoolActive(
                poolId,
                poolId != validatorSetContract.unremovableValidator()
            );
        } else {
            ...
            bool neverStakedBefore = index >= delegatorPoolsLength ||
                delegatorPools[index] != poolId;
            ...
            if (delegatorPoolsLength == 0) {
                ...
                require(
                    validatorSetContract.hasEverBeenMiningAddress(_staker) == 0
                );
            }
        }
    ...
    function _withdraw(
        ...
        address delegatorOrZero = (_staker != _poolStakingAddress)
            ? _staker
            : address(0);
        uint256 newStakeAmount = stakeAmount[poolId][delegatorOrZero].sub(
            _amount
        );
    ...
    function _getDelegatorStake(
        ...
        while (true) {
            delegatorStake = delegatorStakeSnapshot[_poolId][_delegator][
                _epoch
            ];
            if (delegatorStake != 0) {
                delegatorStake = (delegatorStake == uint256(-1))
                    ? 0
                    : delegatorStake;
                break;
    ...
    function _isPoolEmpty(uint256 _poolId) internal view returns (bool) {
        return
            stakeAmountTotal[_poolId] == 0 &&
            orderedWithdrawAmountTotal[_poolId] == 0;
    }
    ...
    function _isPoolToBeElected(uint256 _poolId)
        internal
        view
        returns (bool toBeElected, uint256 index)
    {
        ...
        if (
            ...
            _poolsToBeElected[index] == _poolId
        ) {
            return (true, index);
        }
        return (false, 0);
    ...
    function _isWithdrawAllowed(uint256 _poolId, bool _isDelegator)
        internal
        view
        returns (bool)
    {
